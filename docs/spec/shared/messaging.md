...
| 토픽 패턴   | 용도                             | 발행자      | 구독자      |
| ----------- | -------------------------------- | ----------- | ----------- |
| `heartbeat` | Agent 활성 상태 보고 (20초 주기) | Agent (SDK) | API Service |

Agent 서버는 HTTP를 노출하지 않으므로 heartbeat도 Kafka를 통해 처리한다. Agent가 `heartbeat` 토픽에 메시지를 20초마다 발행하면, API Service가 구독하여 Redis TTL을 갱신한다. Kafka SASL 인증으로 발행자 신원이 보장되므로 별도 검증은 불필요하다.
...
| 필드      | 설명                                   |
| --------- | -------------------------------------- |
| agent_id  | 발행 Agent의 ID (ACL로 자기 ID만 가능) |
| timestamp | 발행 시각 (UTC)                        |
...
> 스케줄 통신의 등록/실행 흐름, 메시지 구조, Scheduler Service 상세는 [스케줄러 문서](../scheduler/scheduler.md) 참고.
...
| api-service      | `heartbeat`              | Read                    |
| agent-a          | `heartbeat`              | Write                   |
...
텔레그램 Bot API는 SSE를 지원하지 않으므로 혼합 방식으로 처리한다. 상세는 [텔레그램 문서](../clients/telegram.md#응답-방식) 참고.
