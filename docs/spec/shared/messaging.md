...
> **Note:** Heartbeat는 Kafka가 아닌 HTTP로 처리된다. Agent가 `POST /agents/{agentName}/heartbeat`를 20초마다 호출하면 API Service가 Redis TTL을 갱신한다. 상세: [Agent Registry 설계](../api/agent-registry.md)
