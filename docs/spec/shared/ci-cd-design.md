...
3. SSH로 OCI VM 접속
4. prod overlay의 이미지 태그를 커밋 SHA로 동적 생성
5. `kubectl apply -k overlays/prod/` (초기 배포 + 업데이트 모두 대응)
6. GCP Parameter Manager에서 `.env.prod` pull → K8s Secret 갱신
7. `kubectl rollout status`로 배포 완료 확인
8. 임시 파일 정리
...
infra/k8s/
  ...
  overlays/
    ...
    prod/                  # OCI K3s용
      kustomization.yaml   # imagePullPolicy: Always, Artifact Registry 이미지 경로
```
...
- **Docker**: Artifact Registry 이미지 pull용
